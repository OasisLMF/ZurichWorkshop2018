...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:agt="http://www.altova.com/Mapforce/agt" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="agt xs">
	...
	<xsl:template name="agt:MapToCatrisks_ModelLoc_var21_function">
		...
		<xsl:variable name="var18_ROWID">
			<xsl:if test="$var20_current/@ROW_ID">
				<xsl:value-of select="'1'"/>
			</xsl:if>
		</xsl:variable>
		<xsl:variable name="var19_resultof_exists" select="boolean(string($var18_ROWID))"/>
		<rec>
			<xsl:if test="string($var19_resultof_exists) != 'false'">
				<xsl:attribute name="Item_ID">
					<xsl:value-of select="string($var20_current/@ROW_ID)"/>
				</xsl:attribute>
			</xsl:if>
	...
	<xsl:template match="/">
		<root>
			<xsl:attribute name="xsi:noNamespaceSchemaLocation" namespace="http://www.w3.org/2001/XMLSchema-instance">C:/Users/Administrator/Desktop/dbFiles/Transformations/git/Catrisks/flamingo/MEEQ/ValidationFiles/Catrisks_ModelLoc.xsd</xsl:attribute>
